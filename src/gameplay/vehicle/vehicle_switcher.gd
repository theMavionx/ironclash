...
## Cycles the active vehicle between tank, helicopter, and drone with the E key.
...
func _ready() -> void:
	...
	# Initialise: tank active, all others inactive.
	if _helicopter:
...
func _toggle_vehicle() -> void:
	# Skip destroyed vehicles. Drone always self-respawns so the loop will
	# always find at least one live vehicle within 3 attempts.
