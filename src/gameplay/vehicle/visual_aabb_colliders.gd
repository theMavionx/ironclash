class_name VisualAABBColliders
extends RefCounted

const GENERATED_GROUP: StringName = &"visual_aabb_collider"
const GENERATED_PREFIX: String = "AutoAABBCollider_"
...
static func rebuild(
	...
	_collect_mesh_boxes(body, visual_root, visual_root, ignore_names, include_hidden, boxes)
	boxes = boxes.filter(func(entry: Dictionary) -> bool:
	...
	if max_shapes > 0 and boxes.size() > max_shapes:
		boxes.resize(max_shapes)

	var built: int = 0
	for entry: Dictionary in boxes:
		var aabb: AABB = entry["aabb"]
		var box_size: Vector3 = Vector3(
			maxf(aabb.size.x + padding.x * 2.0, min_size.x),
			maxf(aabb.size.y + padding.y * 2.0, min_size.y),
			maxf(aabb.size.z + padding.z * 2.0, min_size.z)
		)
...
static func _collect_mesh_boxes(
		body: CollisionObject3D,
		node: Node,
		visual_root: Node,
		ignore_names: PackedStringArray,
		include_hidden: bool,
		out_boxes: Array[Dictionary]
) -> void:
	...
	for child: Node in node.get_children():
		_collect_mesh_boxes(body, child, visual_root, ignore_names, include_hidden, out_boxes)
